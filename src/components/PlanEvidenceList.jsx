...
export default function PlanEvidenceList({ planId }) {
  ...
  const visibleFiles = loadedPlanId === planId ? files : null;
  const load = async () => {
    setBusy(true);
    setError("");
    setFiles(null);
  ...
  return <section className="plan-evidence-list" aria-label="Plan evidence">
    <div className="plan-evidence-heading"><strong>Submitted evidence</strong>
      <button type="button" onClick={load} disabled={busy}>{busy ? "Loading evidence…" : visibleFiles ? "Refresh evidence" : "View evidence"}</button>
    </div>
    {error && <p role="alert">{error}</p>}
    {visibleFiles?.length === 0 && <p>No evidence has been uploaded for this plan.</p>}
    {!!visibleFiles?.length && <>
      <p>Links expire after five minutes. Refresh to open them again.</p>
      <ul>{visibleFiles.map((file) => <li key={file.id}>
